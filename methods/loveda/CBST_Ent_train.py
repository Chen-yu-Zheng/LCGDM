...
def main():
    ...
    for i_iter in tqdm(range(cfg.NUM_STEPS_STOP)):
        if i_iter < cfg.WARMUP_STEP:
            ...
            entloss = entropy_loss(pred_target)

            loss = cls_loss_s * cfg.SOURCE_LOSS_WEIGHT + cls_loss_pse * cfg.PSEUDO_LOSS_WEIGHT + entloss * cfg.PENALTY_LOSS_WEIGHT
            ...
            if i_iter % 50 == 0:
                logger.info('exp = {}'.format(cfg.SNAPSHOT_DIR))
                text = 'Mix iter = %d, loss = %.3f, cls_loss_s = %.3f, cls_loss_pse = %.3f, entropy_loss = %.3f, lr = %.3f'% \
                    (i_iter, loss, cls_loss_s, cls_loss_pse, entloss, lr)
                logger.info(text)
