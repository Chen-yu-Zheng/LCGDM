...
def main():
    ...
    for i_iter in tqdm(range(cfg.NUM_STEPS_STOP)):
        ...
        if i_iter > cfg.WARMUP_STEP:
            penalty_loss, loss_local, loss_global = transfer_loss(target_seg, num_crops=cfg.CROP_NUM)
            loss += cfg.PENALTY_LOSS_WEIGHT * penalty_loss
        ...
        if i_iter % 50 == 0:
            if i_iter <= cfg.WARMUP_STEP:
                ...
                logger.info('iter = %d \t'
                            ...
                            'pseudo_num = %.3f\t'
                            'lr = %.3f' % (
                    i_iter, loss_seg1, loss_pseudo, equalise_cls_loss, penalty_loss, loss_local, loss_global, bounding_num, pseudo_num, lr))
