...
def main():
    ...
    transfer_loss = SBNMLoss(kernel_size=cfg.KERNEL_SIZE)
    best_miou = 0.
    best_iter = 0
    ...
    for i_iter in tqdm(range(cfg.NUM_STEPS_STOP)):
        ...
        loss_penalty = 0
        loss_penalty_local = 0
        loss_penalty_global = 0
        ...
        D_lr = adjust_learning_rate_D(optimizer_D2, i_iter, cfg)
        ...
        for sub_i in range(cfg.ITER_SIZE):
            ...
            if i_iter > cfg.WARMUP_STEP:
                penalty_loss, loss_local, loss_global = transfer_loss(pred_target2, num_crops=cfg.CROP_NUM)
                loss_penalty += penalty_loss.data.cpu().numpy() / cfg.ITER_SIZE
                loss_penalty_local += loss_local.data.cpu().numpy() / cfg.ITER_SIZE
                loss_penalty_global += loss_global.data.cpu().numpy() / cfg.ITER_SIZE
            ...
            if i_iter <= cfg.WARMUP_STEP:
                ...
                loss = cfg.LAMBDA_ADV_TARGET1  * loss_adv_target1 + cfg.LAMBDA_ADV_TARGET2 * loss_adv_target2 + cfg.PENALTY_LOSS_WEIGHT * penalty_loss
        ...
        if i_iter % 50 == 0:
            if i_iter <= cfg.WARMUP_STEP:
                ...
                logger.info('exp = {}'.format(cfg.SNAPSHOT_DIR))
                logger.info(
                    'iter = %d loss_seg1 = %.3f loss_seg2 = %.3f loss_adv1 = %.3f, loss_adv2 = %.3f loss_D1 = %.3f loss_D2 = %.3f loss_penalty = %.3f loss_local = %.3f loss_global = %.3f G_lr = %.5f D_lr = %.5f' % (
                        i_iter, loss_seg_value1, loss_seg_value2, loss_adv_target_value1, loss_adv_target_value2, loss_D_value1, loss_D_value2, loss_penalty, loss_penalty_local, loss_penalty_global, G_lr, D_lr)
                )
