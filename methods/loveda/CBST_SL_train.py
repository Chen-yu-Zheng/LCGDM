...
from utils.tools import COLOR_MAP_LoveDA as COLOR_MAP
...
def main():
    ...
    for i_iter in tqdm(range(cfg.NUM_STEPS_STOP)):
        if i_iter < cfg.WARMUP_STEP:
            ...
            cls_loss_s = loss_calc(pred_source, labels_s['cls'].cuda())
            cls_loss_pse = loss_calc(pred_target, labels_t['cls'].cuda())
            penalty_loss, loss_local, loss_global = transfer_loss(pred_target, num_crops=cfg.CROP_NUM)

            loss = cls_loss_s * cfg.SOURCE_LOSS_WEIGHT + cls_loss_pse * cfg.PSEUDO_LOSS_WEIGHT + penalty_loss * cfg.PENALTY_LOSS_WEIGHT
            ...
            if i_iter % 50 == 0:
                logger.info('exp = {}'.format(cfg.SNAPSHOT_DIR))
                text = 'Mix iter = %d, loss = %.3f, cls_loss_s = %.3f, cls_loss_pse = %.3f, transfer_loss = %.3f, local_loss = %.3f, global_loss = %.3f, lr = %.3f'% \
                    (i_iter, loss, cls_loss_s, cls_loss_pse, penalty_loss, loss_local, loss_global, lr)
                logger.info(text)
...
def val(model, targetloader, save_round_eval_path, cfg):
    ...
    viz_op = VisualizeSegmm(save_pred_vis_path, palette)
...
def label_selection(cls_thresh, image_name_tgt_list, round_idx, save_prob_path, save_pred_path, save_pseudo_label_path, save_pseudo_label_color_path, save_round_eval_path, logger):
    logger.info('###### Start pseudo-label selection in round {} ! ######'.format(round_idx))
    start_pl = time.time()
