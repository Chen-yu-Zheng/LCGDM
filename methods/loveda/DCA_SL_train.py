...
from methods.loveda.eval import evaluate
from utils.tools import *
from utils.my_tools import *

from module.Encoder import Deeplabv2_DCA
from module.my_modules import *
from data.loveda import LoveDALoader
from utils.tools import COLOR_MAP_LoveDA
...
from utils.tools import SBNMLoss

palette = np.asarray(list(COLOR_MAP_LoveDA.values())).reshape((-1,)).tolist()
parser = argparse.ArgumentParser(description='Run CDA methods.')
...
def main():
    ...
    transfer_loss = SBNMLoss(kernel_size=cfg.KERNEL_SIZE, weight_local=cfg.WEIGHT_LOCAL, weight_global=cfg.WEIGHT_GLOBAL)
    ...
    for i_iter in tqdm(range(cfg.NUM_STEPS_STOP)):
        if i_iter <= cfg.FIRST_STAGE_STEP:
            ...
            loss_seg = loss_calc_dca([preds1, preds2], labels_s['cls'].cuda(), multi=True)
            ...
            if i_iter < cfg.NUM_STEPS_STOP and targetloader is not None:
                ...
                # loss
                loss_seg = loss_calc_dca([pred_s1, pred_s2], lab_s, multi=True)
                loss_pseudo = loss_calc_dca([pred_t1, pred_t2], lab_t, multi=True)
                ...
                penalty_loss, loss_local, loss_global = transfer_loss(pred_t2, num_crops=cfg.CROP_NUM)

                loss = loss_seg + loss_pseudo + (source_intra + domain_cross)
                loss += penalty_loss * cfg.PENALTY_LOSS_WEIGHT
                ...
                if i_iter % 50 == 0:
                    ...
                    text = 'iter = %d, total = %.3f, seg = %.3f, pseudo = %.3f, ' \
                           'sour_intra = %.3f, cross = %.3f, transfer_loss = %.3f, local_loss = %.3f, global_loss = %.3f, lr = %.3f' % \
                           (i_iter, loss, loss_seg, loss_pseudo,
                            source_intra, domain_cross, penalty_loss, loss_local, loss_global, lr)
                    logger.info(text)
