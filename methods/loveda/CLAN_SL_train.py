...
# =============================================================================
    ...
    for i_iter in tqdm(range(cfg.NUM_STEPS_STOP)):
        ...
        if i_iter > cfg.WARMUP_STEP:
            loss_penalty, loss_local, loss_global = transfer_loss(F.softmax(pred_target1 + pred_target2, dim = 1), num_crops=cfg.CROP_NUM)
            loss_penalty = loss_penalty * cfg.PENALTY_LOSS_WEIGHT
            (loss_adv + loss_penalty).backward()
        else:
            loss_adv.backward()
        ...
        if i_iter % 50 == 0:
            ...
            if i_iter > cfg.WARMUP_STEP:
                text = 'iter = %d, loss_seg = %.4f loss_adv = %.4f, loss_weight = %.4f, loss_D_s = %.4f loss_D_t = %.4f loss_penalty = %.4f loss_global = %.4f loss_local = %.4f G_lr = %.5f D_lr = %.5f' % \
                (i_iter, loss_seg, loss_adv, loss_weight, loss_D_s, loss_D_t, loss_penalty, loss_global, loss_local, lr, lr_D)
            else:
        ...
        if i_iter >= cfg.NUM_STEPS_STOP - 1:
            ...
            D_path = osp.join(cfg.SNAPSHOT_DIR, cfg.TARGET_SET + str(cfg.NUM_STEPS_STOP) + '_D.pth')
        ...
        if i_iter % cfg.SAVE_PRED_EVERY == 0 and i_iter != 0:
            ...
            D_path = osp.join(cfg.SNAPSHOT_DIR, cfg.TARGET_SET + str(i_iter) + '_D.pth')
            miou = evaluate(model, cfg, True, ckpt_path, logger)
            if miou >= best_miou:
                best_miou = miou
                best_iter = i_iter
