...
def evaluate(model, cfg, is_training=False, ckpt_path=None, logger=None):
    ...
    if cfg.SNAPSHOT_DIR is not None:
        ...
        viz_op = VisualizeSegmm(vis_dir, palette_loveda)
    if not is_training:
    ...
    metric_op = er.metric.PixelMetric(len(COLOR_MAP_LoveDA.keys()), logdir=cfg.SNAPSHOT_DIR, logger=logger)
    with torch.no_grad():
