...
def main():
    ...
    for i_iter in tqdm(range(cfg.NUM_STEPS_STOP)):
        ...
        ## Our Loss
        entloss = entropy_loss(tar_pred) * cfg.PENALTY_LOSS_WEIGHT
        (loss_adv_tgt + entloss).backward()
        ...
        if i_iter % 50 == 0:
            ...
            text = 'i_iter = %d, loss_seg = %.4f loss_adv = %.4f, loss_D_s = %.4f loss_D_t = %.4f loss_ent = %.4f G_lr = %.5f D_lr = %.5f' % (
                    i_iter, loss_seg, loss_adv_tgt, loss_D_src, loss_D_tgt, entloss,lr, lr_D)
            logger.info(text)
