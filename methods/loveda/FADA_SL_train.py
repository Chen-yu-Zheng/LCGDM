...
parser = argparse.ArgumentParser(description='Run FADA methods.')
...
def main():
    ...
    for i_iter in tqdm(range(cfg.NUM_STEPS_STOP)):
        ...
        if i_iter > cfg.WARMUP_STEP:
            loss_penalty, loss_local, loss_global = transfer_loss(tar_pred, num_crops=cfg.CROP_NUM)
            loss_penalty = loss_penalty * cfg.PENALTY_LOSS_WEIGHT
            (loss_adv_tgt + loss_penalty).backward()
        else:
        ...
        if i_iter % 50 == 0:
            if i_iter <= cfg.WARMUP_STEP:
                ...
                logger.info('exp = {}'.format(cfg.SNAPSHOT_DIR))
                text = 'i_iter = %d, loss_seg = %.4f loss_adv = %.4f, loss_transfer = %.4f, loss_global = %.4f, loss_local = %.4f, loss_D_s = %.4f loss_D_t = %.4f G_lr = %.5f D_lr = %.5f' % (
                    i_iter, loss_seg, loss_adv_tgt, loss_penalty, loss_global, loss_local, loss_D_src, loss_D_tgt, lr, lr_D)
                logger.info(text)
