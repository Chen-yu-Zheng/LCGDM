...
def loss_calc_dca(pred, label, reduction='mean', multi=False):
    """
    This function returns dca loss
    """
...
palette_loveda = np.asarray(list(COLOR_MAP_LoveDA.values())).reshape((-1,)).tolist()


    
#IAST generate_pseudo
from utils.viz import VisualizeSegmm
import warnings
...
def generate_pseudo(model, target_loader, save_dir, n_class=7, pseudo_dict=dict(), logger=None):
    ...
    viz_op = VisualizeSegmm(os.path.join(save_dir, 'vis'), palette_loveda)
...
class SBNMLoss(nn.Module):
    def __init__(self, kernel_size=10, weight_local=0.5, weight_global=0.5):
        super(SBNMLoss, self).__init__()
        self.kernel_size = kernel_size
        self.weight_local = weight_local
        self.weight_global = weight_global

    def forward(self, logits, num_crops=10):
        # Outputs of model haven't been softmax
        logits = torch.softmax(logits, dim=1)
        loss_local = 0
        overview = None
        for i in range(num_crops):
            crop = RandomCrop(self.kernel_size)(logits)
            crop = crop.reshape(crop.shape[0], crop.shape[1], -1)
            # print(crop[0])
            loss_local += torch.mean(torch.sqrt(torch.sum(crop ** 2, dim= (1,2))))
            # print('crop_iter = %d, loss_local = %.3f'% (i, loss_local))

            if overview == None:
                overview = torch.softmax(torch.sum(crop, dim=2), dim=1).unsqueeze(dim=1)
            else:
                overview = torch.cat((overview, torch.softmax(torch.sum(crop, dim=2), dim=1).unsqueeze(dim=1)), dim=1)
        
        loss_local = loss_local / num_crops
        # print('loss_local = %.3f'% (loss_local))

        _, singular, _ = torch.svd(overview)
        loss_global = torch.mean(torch.sum(singular, dim=1))
        loss = self.weight_global * loss_global + self.weight_local * loss_local
        return -loss, -loss_local, -loss_global
...
if __name__ == '__main__':
    ...
    net = SBNMLoss(kernel_size=3)
    net = net.double().to(device)
    inputs = torch.rand((2, 6, 10, 10), requires_grad=True, dtype=torch.double).to(device)
    print(net(inputs))
